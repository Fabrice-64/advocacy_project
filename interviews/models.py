...
class AdvocacyTopic(models.Model):
    ...
    is_active = models.BooleanField(
        verbose_name="Thème actif", null=True)
    ...
    key_statement = models.CharField(
        max_length=240,
        blank=False, null=False,
        verbose_name="Phrase Clé")
    slug = models.SlugField(
        max_length=250, unique_for_date='creation_date', null=True)
    source_title = models.CharField(
        max_length=140, null=True, blank=True,
        verbose_name="Titre de la Source")
    source = models.URLField(
        max_length=240,
        blank=True, verbose_name="Lien vers la source")
    ...
    def get_absolute_url(self):
        return reverse(
            'interviews:advocacy_topic_detail',
            args=[str(self.slug)])
...
class Interview(models.Model):
    ...
    date_planned = models.DateField(
        verbose_name="Date prévue", blank=True, null=True)
    date_effective = models.DateField(
        verbose_name="Date de réalisation", blank=True, null=True)
    ...
    volunteer = models.ForeignKey(
        Volunteer,
        on_delete=models.CASCADE, verbose_name="Bénévole",
        null=True, blank=True)
    official = models.ForeignKey(
        Official,
        on_delete=models.CASCADE, verbose_name="Elu")
    topics = models.ManyToManyField(
        AdvocacyTopic,
        verbose_name="Thèmes", blank=True)
